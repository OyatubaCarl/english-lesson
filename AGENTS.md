...
| Notification 音フック（~/.Codex/settings.json） | 完了（PC ローカル） |
...
## 別 PC で再開する手順
1. Google Drive 同期完了を確認（`G:\マイドライブ\個人用\Codex\英語学習教材作成\` 以下が揃っているか）
2. Codex をこのディレクトリで起動 → このファイル（AGENTS.md）を自動読み込み
3. ユーザが「H40 から続けて」または「次へ」と指示
4. Codex は H40 のセクションを読み、上の手順で 3 レッスンずつ進める
