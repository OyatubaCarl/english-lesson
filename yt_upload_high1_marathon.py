#!/usr/bin/env python3
"""Upload high-school grammar all-in-one videos and record their YouTube IDs."""
...
MANIFEST = ROOT / "dist" / "high1-marathon" / "manifest.json"
RESULTS = ROOT / "dist" / "high1-marathon" / "youtube_upload_results.json"
...
def title_for(part: dict) -> str:
    return f"{part['title']}｜うたとミックス 高校英語"


def description_for(part: dict, uploaded: dict[str, dict]) -> str:
    lessons = f"H{part['start']}〜H{part['end']}"
    lines = [
        f"高校編1（文法） {lessons} のレッスン動画を続けて見られる一気見版です。",
        "",
        f"📖 サイトで英文・日本語訳・発音・単語テストを見る: {SITE_URL}?book=high1&lesson={part['start']}",
        "",
        "▼ この動画に入っているレッスン",
    ]
    ...
    lines += [
        ...
        f"チャンネル: {CHANNEL_URL}",
        "",
        "#英語学習 #高校英語 #英文法 #英語リスニング #一気見 #Suno #revidai",
    ]
    return "\n".join(lines)
