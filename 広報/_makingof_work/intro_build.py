#!/usr/bin/env python3
"""アプリ紹介ショート (1080x1920, 30fps, 7カット) ビルダー。

前提: intro_narr1..7.wav / intro_slides.html / 画像素材 が同ディレクトリに生成済み。
出力: ../app_intro_short.mp4
"""
...
OUT = WORK.parent / "app_intro_short.mp4"
...
MIN_CUT = 8.2          # 各カット最低尺 (7カットで合計55秒以上を確保)
MIN_LAST = 8.8         # 最終カットは少し長めに余韻
...
def main():
    ...
    for i in range(1, N_CUTS + 1):
        png = WORK / f"intro_slide{i}.png"
        if not png.exists():
            run(["node", RENDER_JS, f"file://{WORK}/intro_slides.html?cut={i}", str(png)])
            print(f"  ok intro_slide{i}.png")

    # 2) カット尺 = max(ナレ+LEAD_IN+0.8, MIN_CUT)
    cut_secs: list[float] = []
    narr_secs: list[float] = []
    for i in range(1, N_CUTS + 1):
        nd = dur(WORK / f"intro_narr{i}.wav")
        narr_secs.append(nd)
    ...
    for i in range(1, N_CUTS + 1):
        pre = WORK / f"intro_s_pre{i}.wav"
        silence(LEAD_IN, pre)
        tail_len = cut_secs[i - 1] - LEAD_IN - narr_secs[i - 1]
        tail = WORK / f"intro_s_tail{i}.wav"
        silence(max(tail_len, 0.05), tail)
        voice_parts += [pre, WORK / f"intro_narr{i}.wav", tail]
    concat_audio(voice_parts, WORK / "intro_voice.wav")
    vd = dur(WORK / "intro_voice.wav")
    print(f"  intro_voice.wav: {vd:.2f}s")

    # 4) 映像: 各スライド→ズームクリップ→連結
    clips = []
    for i in range(1, N_CUTS + 1):
        clip = WORK / f"intro_clip{i}.mp4"
        img_clip_zoom(WORK / f"intro_slide{i}.png", cut_secs[i - 1], clip)
        clips.append(clip)
        print(f"  ok intro_clip{i}.mp4")
    concat_video(clips, WORK / "intro_base.mp4")
    ...
    run(["ffmpeg", "-y", "-i", str(WORK / "intro_base.mp4"), "-i", str(WORK / "intro_voice.wav"),
