#!/usr/bin/env python3
"""Build the H085 102-cut video plan and verify lyric-to-image alignment."""
...
def purpose(image: Path) -> str:
    ...
    if not match:
        raise RuntimeError(f"unexpected H085 scene filename: {image.name}")
...
def main() -> None:
    ...
    if abs(duration - 131.120167) > 0.02:
        raise RuntimeError(f"unexpected H085 adopted duration: {duration}")
    if timing.get("exact_word_match") is not True or int(timing["word_count"]) != 196:
        raise RuntimeError("H085 TacoBeat timing is not exact")

    unit = duration / SCENE_COUNT
    images = sorted((LESSON / "scenes/final_style01").glob("[0-9][0-9][0-9]_*.jpg"))
    if len(images) != SCENE_COUNT:
        raise RuntimeError(f"H085 scene count: {len(images)} != {SCENE_COUNT}")
    scenes = [
        {
    ...
    if min(gaps) < 1.284 or max(gaps) > 1.287:
    ...
    for item in bible["masters"]:
        ...
        if scene["purpose"] != expected:
            raise RuntimeError(
                f"H085 alignment at {check_time:.2f}s: {scene['purpose']} != {expected}"
            )

    if len(mixed["lines"]) != len(timing["lines"]) or len(mixed["lines"]) != 14:
        raise RuntimeError("H085 caption/timing line count mismatch")
    plan["title_en"] = "Education and Inequality"
    plan["scenes"] = scenes
    plan["captions"] = [
        {
    ...
    plan["qa_status"] = (
        "READY_TO_RENDER_STYLE01_GLOBAL_EDUCATION_EVIDENCE_DIGNITY_AND_TECH_EQUITY_REVIEWED"
    )
    plan["output_name"] = "h085_adopted_song_cinematic_v1.mp4"
    PLAN.write_text(json.dumps(plan, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(
        f"H085 video plan: {len(scenes)} scenes, min {min(gaps):.3f}s "
