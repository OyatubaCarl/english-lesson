#!/usr/bin/env python3
"""Build the H090 129-cut plan and verify lyric-to-image alignment."""
...
def purpose(image: Path) -> str:
    ...
    if not match:
        raise RuntimeError(f"unexpected H090 scene filename: {image.name}")
...
def main() -> None:
    ...
    if abs(duration - 154.640167) > 0.02:
        raise RuntimeError(f"unexpected H090 adopted duration: {duration}")
    if timing.get("exact_word_match") is not True or int(timing["word_count"]) != 197:
        raise RuntimeError("H090 TacoBeat timing is not exact")
    ...
    if len(images) != SCENE_COUNT or len(schedule) != SCENE_COUNT:
        raise RuntimeError(f"H090 scene count: images={len(images)} schedule={len(schedule)}")
    scenes = []
    for (start, expected_purpose), image in zip(schedule, images):
        actual_purpose = purpose(image)
        if actual_purpose != expected_purpose:
            raise RuntimeError(f"H090 purpose mismatch: {actual_purpose} != {expected_purpose}")
        scenes.append({
            "start": round(start, 3),
    ...
    if min(gaps) < 0.73 or max(gaps) > 1.501:
    ...
    for item in bible["masters"]:
        ...
        if scene["purpose"] != expected:
            raise RuntimeError(
                f"H090 alignment at {check_time:.2f}s: {scene['purpose']} != {expected}"
            )

    if len(mixed["lines"]) != len(timing["lines"]) or len(mixed["lines"]) != 18:
        raise RuntimeError("H090 caption/timing line count mismatch")
    plan["title_en"] = "The Scientific Method"
    plan["scenes"] = scenes
    plan["captions"] = [
        {
    ...
    plan["qa_status"] = (
        "READY_TO_RENDER_STYLE01_ITERATIVE_SCIENTIFIC_METHOD_AND_SAFE_EXPERIMENT_REVIEWED"
    )
    plan["output_name"] = "h090_adopted_song_cinematic_v1.mp4"
    PLAN.write_text(json.dumps(plan, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(
        f"H090 video plan: {len(scenes)} scenes, min {min(gaps):.3f}s "
        f"max {max(gaps):.3f}s; 52 semantic checkpoints PASS"
    )
