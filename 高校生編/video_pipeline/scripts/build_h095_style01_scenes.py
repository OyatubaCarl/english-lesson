#!/usr/bin/env python3
"""Create H095 dense Style01 JPEG crops while preserving master PNGs."""
...
def schedule() -> list[tuple[float, str, str, str, int]]:
    ...
    if len(rows) != SCENE_COUNT:
        raise RuntimeError(f"H095 schedule count: {len(rows)} != {SCENE_COUNT}")
    return rows
...
def main() -> None:
    rows = schedule()
    bible = json.loads(VISUAL_BIBLE.read_text(encoding="utf-8"))
    if len(bible["masters"]) != 31:
        raise RuntimeError(f"H095 master count: {len(bible['masters'])} != 31")
    OUT.mkdir(parents=True, exist_ok=True)
    if any(OUT.iterdir()):
        raise RuntimeError(f"H095 output directory must be empty before rebuilding: {OUT}")
    ...
    if actual != expected or len(actual) != SCENE_COUNT or len(usage) != 31 or underused:
        raise RuntimeError(
            f"H095 scene mismatch actual={len(actual)} expected={SCENE_COUNT} "
            f"masters={len(usage)} underused={underused}"
        )
    print(
        f"H095 final_style01 files verified: {len(actual)}; masters used: {len(usage)}; "
