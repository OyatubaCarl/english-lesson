#!/usr/bin/env python3
"""Build the H096 96-cut plan and verify lyric-to-image alignment."""
...
def purpose(image: Path) -> str:
    ...
    if not match:
        raise RuntimeError(f"unexpected H096 scene filename: {image.name}")
...
def main() -> None:
    ...
    if abs(duration - 116.680167) > 0.02:
        raise RuntimeError(f"unexpected H096 adopted duration: {duration}")
    if timing.get("exact_word_match") is not True or int(timing["word_count"]) != 167:
        raise RuntimeError("H096 TacoBeat timing is not exact")
    ...
    if len(images) != SCENE_COUNT or len(schedule) != SCENE_COUNT:
        raise RuntimeError(f"H096 scene count: images={len(images)} schedule={len(schedule)}")
    scenes = []
    for (start, expected_purpose), image in zip(schedule, images):
        actual_purpose = purpose(image)
        if actual_purpose != expected_purpose:
            raise RuntimeError(f"H096 purpose mismatch: {actual_purpose} != {expected_purpose}")
        scenes.append({
            "start": round(start, 3),
    ...
    if min(gaps) < 0.5 or max(gaps) > 1.501:
    ...
    for item in bible["masters"]:
        ...
        if scene["purpose"] != expected:
            raise RuntimeError(
                f"H096 alignment at {check_time:.2f}s: {scene['purpose']} != {expected}"
            )

    if len(mixed["lines"]) != len(timing["lines"]) or len(mixed["lines"]) != 13:
        raise RuntimeError("H096 caption/timing line count mismatch")
    plan["title_en"] = "International Relations and Diplomacy"
    plan["transition"] = 0.28
    plan["scenes"] = scenes
    plan["captions"] = [
        {
    ...
    plan["qa_status"] = "READY_TO_RENDER_STYLE01_MULTILATERAL_DIPLOMACY_NUCLEAR_RISK_REVIEWED"
    plan["output_name"] = "h096_adopted_song_cinematic_v1.mp4"
    PLAN.write_text(json.dumps(plan, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(
        f"H096 video plan: {len(scenes)} scenes, min {min(gaps):.3f}s "
        f"max {max(gaps):.3f}s; 34 semantic checkpoints PASS"
    )
