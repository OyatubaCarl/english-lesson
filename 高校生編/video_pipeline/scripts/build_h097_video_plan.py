#!/usr/bin/env python3
"""Build the H097 96-cut plan and verify lyric-to-image alignment."""
...
def purpose(image: Path) -> str:
    ...
    if not match:
        raise RuntimeError(f"unexpected H097 scene filename: {image.name}")
...
def main() -> None:
    ...
    if abs(duration - 117.840979) > 0.02:
        raise RuntimeError(f"unexpected H097 adopted duration: {duration}")
    if timing.get("exact_word_match") is not True or int(timing["word_count"]) != 154:
        raise RuntimeError("H097 TacoBeat timing is not exact")
    ...
    if len(images) != SCENE_COUNT or len(schedule) != SCENE_COUNT:
        raise RuntimeError(f"H097 scene count: images={len(images)} schedule={len(schedule)}")
    scenes = []
    for (start, expected_purpose), image in zip(schedule, images):
        actual_purpose = purpose(image)
        if actual_purpose != expected_purpose:
            raise RuntimeError(f"H097 purpose mismatch: {actual_purpose} != {expected_purpose}")
        scenes.append({
            "start": round(start, 3),
    ...
    for item in bible["masters"]:
        ...
        if scene["purpose"] != expected:
            raise RuntimeError(
                f"H097 alignment at {check_time:.2f}s: {scene['purpose']} != {expected}"
            )

    if len(mixed["lines"]) != len(timing["lines"]) or len(mixed["lines"]) != 14:
        raise RuntimeError("H097 caption/timing line count mismatch")
    plan["title_en"] = "Environment and Animals"
    ...
    plan["qa_status"] = "READY_TO_RENDER_STYLE01_BIODIVERSITY_COMMUNITY_LED_CONSERVATION_REVIEWED"
    plan["output_name"] = "h097_adopted_song_cinematic_v1.mp4"
    PLAN.write_text(json.dumps(plan, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(
        f"H097 video plan: {len(scenes)} scenes, min {min(gaps):.3f}s "
