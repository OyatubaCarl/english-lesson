#!/usr/bin/env python3
"""Build the H099 112-cut plan and verify lyric-to-image alignment."""
...
def purpose(image: Path) -> str:
    ...
    if not match:
        raise RuntimeError(f"unexpected H099 scene filename: {image.name}")
...
def main() -> None:
    ...
    if abs(duration - 134.560167) > 0.02:
        raise RuntimeError(f"unexpected H099 adopted duration: {duration}")
    if timing.get("exact_word_match") is not True or int(timing["word_count"]) != 160:
        raise RuntimeError("H099 TacoBeat timing is not exact")
    ...
    if len(images) != SCENE_COUNT or len(schedule) != SCENE_COUNT:
        raise RuntimeError(f"H099 scene count: images={len(images)} schedule={len(schedule)}")
    scenes = []
    for (start, expected_purpose), image in zip(schedule, images):
        actual_purpose = purpose(image)
        if actual_purpose != expected_purpose:
            raise RuntimeError(f"H099 purpose mismatch: {actual_purpose} != {expected_purpose}")
        scenes.append({
            "start": round(start, 3),
    ...
    for item in bible["masters"]:
        ...
        if scene["purpose"] != expected:
            raise RuntimeError(
                f"H099 alignment at {check_time:.2f}s: {scene['purpose']} != {expected}"
            )

    if len(mixed["lines"]) != len(timing["lines"]) or len(mixed["lines"]) != 18:
        raise RuntimeError("H099 caption/timing line count mismatch")
    plan["title_en"] = "Life and Choices"
    ...
    plan["qa_status"] = "READY_TO_RENDER_STYLE01_NONDETERMINISTIC_CHOICE_SUPPORT_REFLECTION_REVIEWED"
    plan["output_name"] = "h099_adopted_song_cinematic_v1.mp4"
    PLAN.write_text(json.dumps(plan, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(
        f"H099 video plan: {len(scenes)} scenes, min {min(gaps):.3f}s "
        f"max {max(gaps):.3f}s; 39 semantic checkpoints PASS"
    )
