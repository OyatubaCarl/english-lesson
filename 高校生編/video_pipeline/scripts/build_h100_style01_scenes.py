#!/usr/bin/env python3
"""Create H100 dense Style01 JPEG crops while preserving master PNGs."""
...
LESSON = ROOT / "H100"
MASTERS = LESSON / "scenes/masters_style01"
...
def schedule() -> list[tuple[float, str, str, str, int]]:
    ...
    if len(rows) != SCENE_COUNT:
        raise RuntimeError(f"H100 schedule count: {len(rows)} != {SCENE_COUNT}")
    return rows
...
def main() -> None:
    rows = schedule()
    bible = json.loads(VISUAL_BIBLE.read_text(encoding="utf-8"))
    if len(bible["masters"]) != 47:
        raise RuntimeError(f"H100 master count: {len(bible['masters'])} != 47")
    OUT.mkdir(parents=True, exist_ok=True)
    if any(OUT.iterdir()):
        raise RuntimeError(f"H100 output directory must be empty before rebuilding: {OUT}")
    for _, filename, _, _, _ in rows:
        if not (MASTERS / filename).is_file():
            raise FileNotFoundError(MASTERS / filename)
    ...
    for ordinal, (_, filename, prefix, anchor, crop_number) in enumerate(rows, start=1):
        ...
        render(MASTERS / filename, destination, crop)
    ...
    if actual != expected or len(actual) != SCENE_COUNT or len(usage) != 47 or underused:
        raise RuntimeError(
            f"H100 scene mismatch actual={len(actual)} expected={SCENE_COUNT} "
            f"masters={len(usage)} underused={underused}"
        )
    print(
        f"H100 final_style01 files verified: {len(actual)}; masters used: {len(usage)}; "
