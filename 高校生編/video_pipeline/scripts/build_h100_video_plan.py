#!/usr/bin/env python3
"""Build the H100 120-cut plan and verify lyric-to-image alignment."""
...
def purpose(image: Path) -> str:
    ...
    if not match:
        raise RuntimeError(f"unexpected H100 scene filename: {image.name}")
...
def main() -> None:
    ...
    if abs(duration - 137.560167) > 0.02:
        raise RuntimeError(f"unexpected H100 adopted duration: {duration}")
    if timing.get("exact_word_match") is not True or int(timing["word_count"]) != 172:
        raise RuntimeError("H100 TacoBeat timing is not exact")
    ...
    if len(images) != SCENE_COUNT or len(schedule) != SCENE_COUNT:
        raise RuntimeError(f"H100 scene count: images={len(images)} schedule={len(schedule)}")
    scenes = []
    for (start, expected_purpose), image in zip(schedule, images):
        actual_purpose = purpose(image)
        if actual_purpose != expected_purpose:
            raise RuntimeError(f"H100 purpose mismatch: {actual_purpose} != {expected_purpose}")
        scenes.append({
            "start": round(start, 3),
    ...
    for item in bible["masters"]:
        ...
        if scene["purpose"] != expected:
            raise RuntimeError(
                f"H100 alignment at {check_time:.2f}s: {scene['purpose']} != {expected}"
            )

    if len(mixed["lines"]) != len(timing["lines"]) or len(mixed["lines"]) != 16:
        raise RuntimeError("H100 caption/timing line count mismatch")
    plan["title_en"] = "An Afternoon at a Museum"
    ...
    plan["qa_status"] = "READY_TO_RENDER_STYLE01_FICTIONAL_MUSEUM_ACCESS_CONSERVATION_REVIEWED"
    plan["output_name"] = "h100_adopted_song_cinematic_v1.mp4"
    PLAN.write_text(json.dumps(plan, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(
        f"H100 video plan: {len(scenes)} scenes, min {min(gaps):.3f}s "
        f"max {max(gaps):.3f}s; 47 semantic checkpoints PASS"
    )
